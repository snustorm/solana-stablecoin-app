...
use crate::{CustomError, PRICE_FEED_DECIMAL_ADJUSTMENT};
...
pub fn check_health_factor(
    ...
    if health_factor < config.min_health_factor {
        return Err(CustomError::BelowMinHealthFactor.into());
    }
...
pub fn calculate_health_factor (
    ...
    let collateral_value_in_usd = get_usd_value(
        collateral.lamport_balance, 
        price_feed
    )?;
...
pub fn get_lamports_from_usd(
     amount_in_usd: &u64,
     price_feed: &Account<PriceUpdateV2>) -> Result<u64> {

    let feed_id = get_feed_id_from_hex(FEED_ID)?;
    let price = price_feed.get_price_no_older_than(&Clock::get()?, MAXIMUM_AGE, &feed_id)?;
    
    require!(price.price > 0, CustomError::InvalidPrice);
    let price_in_use = price.price as u128 * PRICE_FEED_DECIMAL_ADJUSTMENT;
    
    let amount_in_lamports = (*amount_in_usd as u128 * LAMPORTS_PER_SOL as u128) / price_in_use;
